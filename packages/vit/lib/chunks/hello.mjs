import { c as commonjsGlobal, g as getDefaultExportFromCjs, a as consola } from './cli.mjs';
import { s as source, o as onetimeExports, a as signalExitExports, i as inheritsExports, c as cliSpinners, w as wcwidthExports } from './inherits.mjs';
...
const figures$1 = platform === 'win32' ? windows : main$1;
...
var isObject$3 = isObject_1,
    isPrototype$2 = require_isPrototype(),
...
var isPrototype$1 = require_isPrototype(),
...
Object.defineProperty(from$3, "__esModule", { value: true });
from$3.from = void 0;
...
Object.defineProperty(of$3, "__esModule", { value: true });
of$3.of = void 0;
var args_1$c = args;
var from_1$6 = from$3;
function of$2() {
...
function mapOneOrManyArgs(fn) {
    return map_1$6.map(function (args) { return callOrApply(fn, args); });
}
...
function mergeMap(project, resultSelector, concurrent) {
    ...
    if (isFunction_1$b.isFunction(resultSelector)) {
        return mergeMap(function (a, i) { return map_1$5.map(function (b, ii) { return resultSelector(a, b, i, ii); })(innerFrom_1$z.innerFrom(project(a, i))); }, concurrent);
    }
...
function combineLatest() {
    ...
    return resultSelector
        ? pipe_1.pipe(combineLatest.apply(void 0, __spreadArray$d([], __read$d(args))), mapOneOrManyArgs_1.mapOneOrManyArgs(resultSelector))
...
Object.defineProperty(concatMap$2, "__esModule", { value: true });
...
function mapTo(value) {
    return map_1$4.map(function () { return value; });
}
mapTo$1.mapTo = mapTo;
...
function distinct(keySelector, flushes) {
    return lift_1$H.operate(function (source, subscriber) {
        ...
        flushes && innerFrom_1$h.innerFrom(flushes).subscribe(OperatorSubscriber_1$w.createOperatorSubscriber(subscriber, function () { return distinctKeys.clear(); }, noop_1$7.noop));
    });
}
...
function endWith() {
    ...
    return function (source) { return concat_1$1.concat(source, of_1.of.apply(void 0, __spreadArray$9([], __read$9(values)))); };
}
...
function exhaustMap(project, resultSelector) {
    if (resultSelector) {
        return function (source) {
            return source.pipe(exhaustMap(function (a, i) { return innerFrom_1$g.innerFrom(project(a, i)).pipe(map_1$3.map(function (b, ii) { return resultSelector(a, b, i, ii); })); }));
        };
    }
...
Object.defineProperty(expand$2, "__esModule", { value: true });
expand$2.expand = void 0;
var lift_1$C = lift;
...
function first(predicate, defaultValue) {
    var hasDefaultValue = arguments.length >= 2;
    return function (source) {
        return source.pipe(predicate ? filter_1$4.filter(function (v, i) { return predicate(v, i, source); }) : identity_1$7.identity, take_1.take(1), hasDefaultValue ? defaultIfEmpty_1$1.defaultIfEmpty(defaultValue) : throwIfEmpty_1$1.throwIfEmpty(function () { return new EmptyError_1$2.EmptyError(); }));
    };
}
first$1.first = first;
...
function last(predicate, defaultValue) {
    var hasDefaultValue = arguments.length >= 2;
    return function (source) {
        return source.pipe(predicate ? filter_1$3.filter(function (v, i) { return predicate(v, i, source); }) : identity_1$6.identity, takeLast_1.takeLast(1), hasDefaultValue ? defaultIfEmpty_1.defaultIfEmpty(defaultValue) : throwIfEmpty_1.throwIfEmpty(function () { return new EmptyError_1$1.EmptyError(); }));
    };
}
last$1.last = last;
...
function onErrorResumeNextWith() {
    ...
    return function (source) { return onErrorResumeNext_1.onErrorResumeNext.apply(void 0, __spreadArray$6([source], __read$6(nextSources))); };
}
...
Object.defineProperty(publish$2, "__esModule", { value: true });
...
function skip(count) {
    return filter_1$2.filter(function (_, index) { return count <= index; });
}
skip$1.skip = skip;
...
function takeUntil(notifier) {
    return lift_1$b.operate(function (source, subscriber) {
        innerFrom_1$5.innerFrom(notifier).subscribe(OperatorSubscriber_1$a.createOperatorSubscriber(subscriber, function () { return subscriber.complete(); }, noop_1$4.noop));
...
function timestamp(timestampProvider) {
    ...
    return map_1$1.map(function (value) { return ({ value: value, timestamp: timestampProvider.now() }); });
}
...
utils$1.fetchAsyncQuestionProperty = function (question, prop, answers) {
  ...
  return from$1(
    runAsync$1(question[prop])(answers).then((value) => {
      question[prop] = value;
      return question;
    })
  );
...
const { defer, empty, from, of } = cjs;
const { concatMap, filter, publish, reduce } = operators;
...
function requireOra () {
	...
	const cliSpinners$1 = cliSpinners;
	...
	class Ora {
		...
		set spinner(spinner) {
			...
			if (typeof spinner === 'object') {
				...
				this._spinner = cliSpinners$1.line;
			} else if (spinner === undefined) {
				// Set default spinner
				this._spinner = cliSpinners$1.dots;
			} else if (spinner !== 'default' && cliSpinners$1[spinner]) {
				this._spinner = cliSpinners$1[spinner];
			} else {
...
function requireBase () {
	...
	const chalk = source;
	const runAsync = runAsyncExports;
	const { filter, flatMap, share, take, takeUntil } = operators;
...
function requireEvents () {
	...
	hasRequiredEvents = 1;
	const { fromEvent } = cjs;
	const { filter, map, share, takeUntil } = operators;
...
function requireList () {
	...
	const cliCursor = requireCliCursor();
	const runAsync = runAsyncExports;
	const { flatMap, map, take, takeUntil } = operators;
...
function requireConfirm () {
	...
	const chalk = source;
	const { take, takeUntil } = operators;
	const Base = requireBase();
	const observe = requireEvents();
...
function requireStreams () {
	...
	var Buffer = require$$0$4.Buffer,
	    Transform = require$$0$1.Transform;
...
const words = ["world", "\u{1D534}\u{1D52C}\u{1D52F}\u{1D529}\u{1D521}", "\u{1F166}\u{1F15E}\u{1F161}\u{1F15B}\u{1F153}", "\u{1D6A0}\u{1D698}\u{1D69B}\u{1D695}\u{1D68D}"];
